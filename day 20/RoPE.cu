...
int main() {
    ...
    size_t size = batch_size * seq_len * num_heads * head_dim * sizeof(float);
    float *d_queries, *d_keys;
    cudaMalloc(&d_queries, size);
    cudaMalloc(&d_keys, size);

    float *h_queries = new float[batch_size * seq_len * num_heads * head_dim];
    float *h_keys = new float[batch_size * seq_len * num_heads * head_dim];

    for(size_t i = 0; i < batch_size * seq_len * num_heads * head_dim; i++) {
        h_queries[i] = static_cast<float>(rand()) / RAND_MAX;
        h_keys[i] = static_cast<float>(rand()) / RAND_MAX;
    }

    cudaMemcpy(d_queries, h_queries, size, cudaMemcpyHostToDevice);
    cudaMemcpy(d_keys, h_keys, size, cudaMemcpyHostToDevice);
